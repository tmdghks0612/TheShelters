...
*/
void AShelterControl::RandomMoveMonster(int monsterId, Direction d, int retry) {
	for (int i=0; i < retry; i++) {
		bool success = MoveMonster(monsterId, Direction(rand() % 4));
		if (success) {
			return;
		}
	}
	if (!MoveMonster(monsterId, Direction(rand() % 4))) {
		MoveMonster(monsterId, Direction(rand() % 4));
	}
}
