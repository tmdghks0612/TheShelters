...
bool ARoomControl::IsNextPanicRoom(int roomNumber) {
  if (roomNumber == panicRoomId - 1 || roomNumber == panicRoomId + 1 ||
      roomNumber == panicRoomId + 10) {
...
void ARoomControl::InitCCTV(TArray<AActor *> _ZapPlanes,
  ...
  for (int i = 0; i < 12; ++i) {
    ...
    FVector spawnLocation(startX + col * interval, startY + row * interval,
                          startZ);
...
Direction
ARoomControl::ChooseWeightedRandomDirection(TMap<Direction, int32> weights) {
...
void ARoomControl::EndTurn() {
  ...
  for (const TPair<int32, int32> &it : monsterLocations) {
    TMap<Direction, int32> weights = {
        {Left, 6}, {Right, 6}, {Up, 6}, {Down, 6}, {NoDirection, 1}};
    ...
    if (row % 2 == 0) {
      for (int i = 0; i < 10; ++i) {
        ...
        if (GameMap[currentRoomId]->GetDoor(Up).connectedRoom != nullptr) {
          GameMap[currentRoomId]->GetDoor(Up).connectedRoom->SetDoor(
              Down, currentDoorStatus);
        }
      }
    }
    // when reading row is left, right info
      ...
      for (int i = 0; i < 10; ++i) {
        ...
        if (GameMap[currentRoomId]->GetDoor(Left).connectedRoom != nullptr) {
          GameMap[currentRoomId]->GetDoor(Left).connectedRoom->SetDoor(
              Right, currentDoorStatus);
        }
      }
    }
    ++row;
...
void ARoomControl::SpawnRoomMesh(int roomNum) {
  ...
  FVector spawnLocation(startX + col * interval, startY + row * interval,
                        startZ);
  ARoomActor *roomActor = GetWorld()->SpawnActor<ARoomActor>(
      RoomActor, spawnLocation, rotator, spawnParams);
...
void ARoomControl::SpawnDoorMesh(int roomNum) {
  ...
  int row = roomNum / maxWidth;
  int col = roomNum % maxWidth;
  UE_LOG(LogTemp, Warning, TEXT("roomnum %d, coord [%d : %d]"), roomNum, row,
         col);
  FActorSpawnParameters spawnParams;
  spawnParams.Owner = this;
  ...
  Door currentDoor;
  FVector spawnLocationUp(startX + col * interval,
                          startY + row * interval - 375.0f, startZ);
  currentDoor = GameMap[roomNum]->GetDoor(Up);
  if (currentDoor.connectedRoom != nullptr && currentDoor.status == Open) {

    world->SpawnActor<ADoorActor>(DoorActor[0], spawnLocationUp, rotatorUp,
                                  spawnParams);
  } else {
    world->SpawnActor<ADoorActor>(DoorActor[1], spawnLocationUp, rotatorUp,
                                  spawnParams);
  }

  FVector spawnLocationLeft(startX + col * interval - 375.0f,
                            startY + row * interval, startZ);
  currentDoor = GameMap[roomNum]->GetDoor(Left);
  if (currentDoor.connectedRoom != nullptr && currentDoor.status == Open) {

    world->SpawnActor<ADoorActor>(DoorActor[0], spawnLocationLeft, rotatorLeft,
                                  spawnParams);
  } else {
    world->SpawnActor<ADoorActor>(DoorActor[1], spawnLocationLeft, rotatorLeft,
                                  spawnParams);
  }

  FVector spawnLocationRight(startX + col * interval + 375.0f,
                             startY + row * interval, startZ);
  currentDoor = GameMap[roomNum]->GetDoor(Right);
  if (currentDoor.connectedRoom != nullptr && currentDoor.status == Open) {

    world->SpawnActor<ADoorActor>(DoorActor[0], spawnLocationRight, rotatorLeft,
                                  spawnParams);
  } else {
    world->SpawnActor<ADoorActor>(DoorActor[1], spawnLocationRight, rotatorLeft,
                                  spawnParams);
  }

  FVector spawnLocationDown(startX + col * interval,
                            startY + row * interval + 375.0f, startZ);
  currentDoor = GameMap[roomNum]->GetDoor(Down);
  if (currentDoor.connectedRoom != nullptr && currentDoor.status == Open) {

    world->SpawnActor<ADoorActor>(DoorActor[0], spawnLocationDown, rotatorUp,
                                  spawnParams);
  } else {
    world->SpawnActor<ADoorActor>(DoorActor[1], spawnLocationDown, rotatorUp,
                                  spawnParams);
  }
}
...
void ARoomControl::InitVisibleRoom() {
  ...
  while (1) {
    ...
    if (currentRoom == nullptr || currentDoor.connectedRoom == nullptr ||
  ...
  for (const TPair<int32, int32> &it : monsterLocations) {
    if (it.Key == _monsterId) {
      ...
      if (roomNumber == panicRoomId - 1) {
        ...
        UE_LOG(LogTemp, Warning, TEXT("monster %d roomNumber error!"),
               _monsterId);
        return false;
      }
      break;
...
URoom *ARoomControl::FindRoomByLocation(const unsigned int x,
...
URoom *ARoomControl::FindRoomById(const int roomId) { return GameMap[roomId]; }
...
void ARoomControl::InsertMonster(MonsterType monsterType, int roomId) {
  ...
  AMonsterActor *newMonster = NewObject<AMonsterActor>();
  ...
  if (MonsterSpawn[monsterType]) {
    ...
    if (world) {
      ...
      FRotator rotator(0.0f, 0.0f, 0.0f);
      FVector spawnLocation(startX + x * interval, startY + y * interval,
                            startZ);

      monsterActors.Add(world->SpawnActor<AMonsterActor>(
          MonsterSpawn[monsterType], spawnLocation, rotator, spawnParams));
  ...
  for (const TPair<int32, int32> &it : monsterLocations) {
    if (it.Key == monsterId) {
      ...
      if (monsterActors[monsterId - 1]) {
        monsterActors[monsterId - 1]->SetActorLocation(
            FVector(startX + interval * (it.Value % maxWidth),
...
AMonsterActor *ARoomControl::FindMonsterById(const unsigned int id) {

  for (const TPair<int32, AMonsterActor *> &it : monsters) {
    if (it.Key == id) {
      UE_LOG(LogTemp, Warning, TEXT("Hello pretty monster"));
      AMonsterActor *m = it.Value;
